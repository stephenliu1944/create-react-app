...
export default function Lazyload(loader: Loader, options: Options = {}): FC {
  const { loading: Spin = Loading, error: Error = ErrorBoundary, delay = 300, timeout = 1000 * 30 } = options;
  const clear = (...ids: number[]) => ids.forEach(i => window.clearTimeout(i));
  ...
  const LazyComponent = lazy(() => {
    ...
    ]).then(_loader => _loader()).catch(err => Promise.reject(new Error(err)));
  });
